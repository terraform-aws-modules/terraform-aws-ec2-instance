...
| <a name="input_network_interface"></a> [network\_interface](#input\_network\_interface) | Customize network interfaces to be attached at instance boot time. `network_interface` is deprecated. Use `primary_network_interface` to specify the primary network interface | <pre>map(object({<br/>    delete_on_termination = optional(bool)<br/>    device_index          = optional(number) # Will fall back to use map key as device index<br/>    network_card_index    = optional(number)<br/>    network_interface_id  = string<br/>  }))</pre> | `null` | no |
...
| <a name="input_primary_network_interface"></a> [primary\_network\_interface](#input\_primary\_network\_interface) | Customize primary network interface on the EC2 Instance | <pre>object({<br/>    network_interface_id = string<br/>  })</pre> | `null` | no |
