...
| ipv6_addresses | Specify one or more IPv6 addresses from the range of the subnet to associate with the primary network interface | list | `<list>` | no |
...
| network_interface | Customize network interfaces to be attached at instance boot time | list | `<list>` | no |
...
| subnet_ids | A list of VPC Subnet IDs to launch in | list | `<list>` | no |
...
| volume_tags | List of volume tags of instances |
